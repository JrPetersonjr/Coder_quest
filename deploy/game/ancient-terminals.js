...
window.AncientTerminal = {
  ...
  async open(terminalId, appendLine) {
    ...
    // Generate challenge
    if (terminal.dynamic && terminal.generateChallenge) {
      this.challenge = terminal.generateChallenge(gameState);
    } else if (terminal.generateChallenge) {
      // Non-dynamic terminals can still have generateChallenge method
      this.challenge = terminal.generateChallenge(gameState);
    } else if (terminal.challenges) {
      this.challenge = terminal.challenges[0];
    } else {
      // Fallback for terminals without proper challenge setup
      this.challenge = {
        id: "fallback-challenge",
        desc: "Terminal interface ready.",
        shortPrompt: "Ready for input.",
        hint: "Type commands to interact with this system."
      };
    }
